...
-- 문제3 직급이 'ST_MAN'인 직원이 받는 급여들의 최소 급여보다 많이 받는 직원들의 
-- 이름과 급여를 출력하되 부서번호가 20번인 직원은 제외한다.
...
-- 문제4 EMPLOYEES 테이블에서 Valli라는 이름을 가진 직원과 업무명 및 월급이 같은 사원의 모든 정보를 출력하는 SELECT문을 작성하시오. 
-- (결과에서 Valli는 제외)
...
-- 문제5 EMPLOYEES 테이블에서 월급이 자신이(‘Valli’) 속한 부서의 평균 월급보다 높은 사원의 
-- 부서번호, 이름, 급여를 출력하는 SELECT문을 작성하시오.
SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE FIRST_NAME = 'Valli';
SELECT AVG(SALARY) FROM EMPLOYEES WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE FIRST_NAME = 'Valli');
...
SELECT FIRST_NAME, LAST_NAME, JOB_ID, SALARY, HIRE_DATE FROM EMPLOYEES WHERE (SALARY, JOB_ID) IN (SELECT MIN(SALARY), JOB_ID FROM EMPLOYEES GROUP BY JOB_ID);

SELECT FIRST_NAME, LAST_NAME, SALARY, DEPARTMENT_ID, JOB_ID FROM EMPLOYEES WHERE SALARY > ANY(SELECT AVG(SALARY) FROM EMPLOYEES GROUP BY DEPARTMENT_ID);

SELECT FIRST_NAME, LAST_NAME, JOB_ID, SALARY, DEPARTMENT_ID, ROUND(AVG(SALARY)) AS "Department Avg Salary" FROM EMPLOYEES (SELECT AVG(SALARY) FROM EMPLOYEES GROUP BY DEPARTMENT_ID);

-- 사원의 급여 정보중 업무별 최소 급여를 받고 있는 사원의 성과 이름, 업무, 급여, 입사일을 출력하시오
-- 업무별 최소급여
select * from employees;
SELECT FIRST_NAME, LAST_NAME, JOB_ID, SALARY, HIRE_DATE FROM EMPLOYEES 
WHERE (SALARY, JOB_ID) IN (SELECT MIN(SALARY) FROM EMPLOYEES GROUP BY JOB_ID);
...
-- 모든 사원의 소속부서 평균급여를 계산하여 사원별로 이름과 업무, 급여, 부서번호, 부서 평균급여(DEPARTMENT_AVG_SALARY)를 출력하시오.
SELECT ROUNG(AVG(SALARY)) FROM EMPLOYEES WHERE DEPARTMENT_ID = 90 GROUP BY DEPARTMENT_ID;
